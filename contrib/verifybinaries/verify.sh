...
###   dzcoincore.org and dzcoin.org and compares them.
...
WORKINGDIR="/tmp/dzcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://dzcoincore.org"
HOST2="https://dzcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="dzcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'dzcoin-',
...
if [ $? -ne 0 ]; then
   echo "dzcoin.org failed to provide signature file, but dzcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "dzcoin.org and dzcoincore.org signature files were not equal?"
